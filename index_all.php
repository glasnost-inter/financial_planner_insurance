			...
			<div class="row" style="margin-top:10px">
				<div class="span6" style="border:1px solid #bbb;border-radius:10px;padding:10px 20px 10px 20px;margin:0 0 10px 0;width:1034px">
					<div class="tabbable tabs-left">
					  ...
					  <!--deploy here-->
					   <?php																
						$sqlx = "select kd_produk from produk";
						//echo $sqlx;		
						$resultx = mysql_query($sqlx);
						$ix = 1;
						while($rowx = mysql_fetch_array($resultx))
						{
						?>
							<div class="tab-pane <?php if ($ix==1){ echo 'active';}?>" id="A<?=$ix;?>">
							...
							<!-- kontainer kolom -->
								...
								<div id="myTabContent" class="tab-content">
								  ...
								  <div class="tab-pane fade in active" id="cashflow_benefit">
									...
									<!-- awal new -->
									<?php
									$sqlx = "SELECT * FROM produk_skema A,skema B WHERE A.kd_produk = '".$rowx['kd_produk']."' AND A.KD_skema = B.KD_skema";
									//echo $sqlx;		
									$resultx = mysql_query($sqlx);
									$i = 1;
									...
									<div class="accordion-group">
										  ...
										  <div id="collapse_cb<?=$i;?>" class="accordion-body collapse <?php if($i==1){ echo 'in';} ?>">		  
											<div class="accordion-inner">							                                
												<div>
												  <blockquote>
												  <p>Jadwal pembayaran manfaat sebagai berikut :</p>
												  </blockquote>	
												</div> <!-- tutup blockquote --> 
												<?php
												$sqlz = "select distinct DATE_FORMAT(TGL_JATUH_TEMPO,'%Y') tgl_jatuh_tempo
														 from
												...
												<div class="accordion" id="accordion2in<?=$kd_produk_skema.$i;?>">			   
												<?php
												$x = 1;
												...
												?>										 
													...
													<div class="accordion-group">
														<div class="accordion-heading">
															<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2in<?=$kd_produk_skema.$i;?>" href="#collapseInner<?=$kd_produk_skema.$x;?>">
															<?=$rowz['tgl_jatuh_tempo'];?>
															</a>
														</div>
														<div id="collapseInner<?=$kd_produk_skema.$x;?>" class="accordion-body collapse">
															<?php
															$sqlz1= "select *,(select nama_benefit from benefit where benefit.kd_benefit = b.kd_benefit) nama_benefit
																	from
																	(SELECT a.kd_benefit,substr(a.kd_produk_skema,length(a.kd_produk_skema)-2,3) jenis_klaim,(select nama_jenis_klaim from jenis_klaim where kd_jenis_klaim = substr(a.kd_produk_skema,length(a.kd_produk_skema)-2,3)) nama_klaim
																											FROM produk_skema_klaim_benefit A,BENEFIT B,cara_bayar c,frekuensi_bayar d
																											WHERE kd_produk_skema in (SELECT kd_produk_skema_klaim 
																	...
																	and b.no_klien = '1'
																	and a.jenis_klaim = b.kd_klaim
																	AND DATE_FORMAT(TGL_JATUH_TEMPO,'%Y') = '".$rowz['tgl_jatuh_tempo']."'
																	order by a.kd_benefit";
															//echo $sql;		
															$resultz1 = mysql_query($sqlz1);
															?>
															...
															<?php
															$x1 = 1;
															while($rowz1 = mysql_fetch_array($resultz1))
															{						
															?>
															<tr>
																<td><?=$x1++;?></td>
																<td><?=$rowz1['nama_benefit'];?></td>
																<td><?=$rowz1['tgl_jatuh_tempo'];?></td>
																<td><?=$rowz1['tgl_pengajuan'];?></td>
																<td><?=$rowz1['tgl_pembayaran'];?></td>
																<td><?=$rowz1['nilai_klaim'];?></td>
																<td><?=$rowz1['potongan'];?></td>
																<td><?=$rowz1['nilai_dibayarkan'];?></td>
																<td><?=$rowz1['kd_metode_bayar'];?></td>
																<td><?=$rowz1['penerima'];?></td>
																<td><?=$rowz1['bukti_pembayaran'];?></td>
															</tr>														  
															<?php
															}						
															?>
								  ...
								  <div class="tab-pane fade" id="kondisi">
									...
									<div class="accordion-group">
									  ...
									  <div id="collapse_ks<?=$i;?>" class="accordion-body collapse <?php if($i==1){ echo 'in';} ?>">						  
										<div class="accordion-inner">							                                
										<div>
										  <blockquote>
